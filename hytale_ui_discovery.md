...
For modular, maintainable UIs, use these patterns:

### Clear and Append

```java
// Clear container before rebuilding
uiCommandBuilder.clear("#MyContainer");

// Append components dynamically
uiCommandBuilder.append("#MyContainer", "Components/MyComponent.ui");
```

### Index-Based Selectors

After appending, target children by index:

```java
// First appended child
uiCommandBuilder.set("#MyContainer[0] #Label.Text", "First Item");

// Second appended child
uiCommandBuilder.set("#MyContainer[1] #Label.Text", "Second Item");
```

### Event Binding with Encoded Data

Pass structured data in events:

```java
uiEventBuilder.addEventBinding(
    CustomUIEventBindingType.ValueChanged,
    "#MyContainer[0] #CheckBox",
    EventData.of("section", "mySection").and("slot", "0"),
    false
);
```
...
Create reusable `.ui` files in a `Components/` folder:

```ui
// Components/ArmorSlotRow.ui
Group {
  LayoutMode: Left;
  $C.@CheckBox #CheckBox {}
  Label #Label { Text: ""; }
...
Then append and configure dynamically from Java.

### Real-Time Updates

Call `sendUpdate()` after state changes to refresh the UI:

```java
@Override
public void handleDataEvent(...) {
    // Handle the event...
    this.sendUpdate(); // Triggers rebuild with new state
}
```
